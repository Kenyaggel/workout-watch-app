...
A watch-first strength workout app. The watch target manages the live workout loop, logs performed sets, uses haptics around rest/set transitions, saves sessions through SwiftData, and integrates with HealthKit without interrupting third-party audio. The iPhone app is currently only a stub.
...
- iOS placeholder UI lives under `WorkoutApp/WorkoutApp`.
